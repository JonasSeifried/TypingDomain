...
function calculateInputFieldClass(): string {
  ...
  return inputField.value.selectionStart - inputField.value.value.length == 0 ? "caret-transparent" : "caret-text";
}
      ...
      <div ref="parentDivOfTypedDivs" class="grid items-end w-1/2 h-fit">
        ...
        >
          <span
            ...
            :key="index"
            :class="[item === wholeText[index] ? 'text-accent' : 'text-rose-600 underline']"
          >
            {{ item }}
          </span>
        </div>
