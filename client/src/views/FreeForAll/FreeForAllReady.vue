...
<template>
  <div class="flex flex-col w-1/2 p-4 m-4">
    ...
    <div v-for="client in roomStore.clientsInRoom" :key="client.socketId" class="flex justify-between w-full">
      <div class="relative w-2/3">
        <span class="inline-block max-w-full text-2xl truncate peer">{{ client.username }}</span>
        <div
      ...
      <i :class="['nf text-2xl', client.isReady ? 'nf-oct-check text-accent' : 'nf-oct-x text-rose-600']"></i>
    </div>
  </div>
</template>
