...
io.on('connection', (socket) => {
    ...
    socket.on('roomSetReady', (isReady: boolean) => {
        ...
        console.log(`User ${socket.id} is ready: ${isReady}`)
        ...
        if (rooms.roomReady(roomId).unwrapOr(false)) {
...
function onTypedTextChanged(socket: Socket, text: string) {
    ...
    if (cappedText.length == roomText.length) {
        ...
        console.log(`User ${socket.id} finished`)
        socket.emit('playerFinished')
        if (rooms.allPlayersFinished(roomId).unwrapOr(false)) {
            console.log(`All players finished in room ${roomId}`)
...
function onClientDataChanged(roomId: string) {
    io.to(roomId).emit('clientDataInRoomChanged', fromResult(rooms.getClientDataOfRoom(roomId)))
}

function onRoomDataChanged(roomId: string) {
    io.to(roomId).emit('roomDataChanged', fromResult(buildClientRoomData(roomId)))
}
...
function startRoomPlayTimer(roomId: string) {
    ...
    const roomPlayTimerInterval = setInterval(() => {
        ...
        if (!rooms.hasRoom(roomId) || result.isErr() || result.value === GameState.POSTGAME) {
