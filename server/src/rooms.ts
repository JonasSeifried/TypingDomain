...
export class Rooms {
    ...
    public joinRoom(roomId: string, socketId: string, username: string): Result<void> {
    ...
    public allPlayersFinished(roomId: string): Result<boolean> {
        const result = this.getRoom(roomId)
        if (result.isErr()) return result
        return ok(!Array.from(result.value.players).some(([, player]) => !player.isFinished))
    }
    ...
    public roomReady(roomId: string): Result<boolean> {
        ...
        if (result.isErr()) return result
        return ok(
            !Array.from(result.value.players).some(([, clientData]) => clientData.isReady === false)
        )
    }
    ...
    private getRoomOfClient(socketId: string): Result<RoomData> {
        ...
        return !roomId ? err(Error(`Client "${socketId}" is not in a room`)) : this.getRoom(roomId)
    }
